...
lpaq1是PAQ的“精简”版本，以压缩为代价（比paq8l快35倍）（但类似于高端BWT和PPM压缩器）。
它是一个上下文混合压缩器，它组合了7个上下文：顺序1、2、3、4、6，一个小写字母字符上下文（用于ASCII文本）和一个“匹配”顺序，该顺序预测最后一个匹配上下文中的下一位 。
使用80个神经网络之一（由小上下文选择）组合7个模型的独立位预测，然后使用2个SSE阶段（0和1阶）进行调整并进行算术编码。
...
预测是按位的。这意味着n阶上下文由最后n个完整字节加上当前字节的0至7个先前编码的位（从最高有效位开始）组成。
字母组合词上下文由最后（最多）11个连续字母（A-Z，a-z）的哈希值折叠成小写字母组成。 
上下文不包括任何非字母字符或最后一个非字母字符之前的任何字符。
...
前6个上下文（顺序1..4、6，字）用于索引哈希表，以查找由8位状态表示的位历史。
可表示的状态与paq8l中的相同。 状态可以代表最长4位长的所有历史记录，或者一对0,1计数加上一个标志以指示最新位。 
计数以（41,0），（40,1），（12,2），（5,3），（4,4）为边界，同样以1,0为边界。 
超过计数时，将减少相反的计数以大致保留计数比率。 最后一个位标志仅在总数小于16的状态中存在。有253种可能的状态。
...
使用自适应表（StateMap）将位历史映射到概率。 这与paq81的不同之处在于，每个表条目都包含一个计数，因此刚开始适应很快。 
每个表条目具有22位的概率（最初为p = 0.5）和10位的计数（最初为n = 0）打包为32位。 
在预测了位y之后，将n增加到极限（1023），并通过p：= p +（y-p）/（n + 0.5）调整概率。 
该模型是平稳的：p =（n1 + 0.5）/（n + 1），其中n1是n中y = 1的次数。
...
“匹配”模型（MatchModel）在哈希表中查找当前上下文，首先使用较长的上下文，然后使用较短的上下文。
如果找到匹配项，则将预测以下位，直到出现错误预测为止。 
通过将预测位，匹配的长度（1..15或16..62中的4量化，最大为62）以及最后一个完整字节作为上下文映射到StateMap中来计算预测。 
如果找不到匹配项，则将顺序0上下文（当前字节的后0..7位）用作StateMap的上下文。
...
与paq8l中一样，使用神经网络（Mixer）组合了这7个预测，只是它是没有MMX代码的单级网络。
首先拉伸输入p_i，i = 0..6：t_i = log（p_i /（1-p_i）），然后计算输出：p = squash（SUM_i t_i * w_i），其中squash（x）= 1 /（1 + exp（-x））是Stretch（）的反函数。 
调整权重以减少误差：w_i：= w_i + L * t_i *（y-p）其中（y-p）是预测误差，L〜0.002是学习率。
这是一个标准的单层反向传播网络，经过修改可最大程度地减少编码成本，而不是RMS预测误差（因此从学习中减去因素p *（1-p））。
...
根据上下文选择80个神经网络之一，该上下文取决于最后一个完整字节的3个高位位加上上下文顺序（量化为0、1、2、3、4、6、8、12、16、32） 。 
顺序由非零位历史记录的数量和MatchModel中的匹配长度决定。
...
混合器输出由2个SSE阶段调整（自适应概率图称为APM）。 APM是一种StateMap，它既接受离散上下文又接受输入概率pr。
pr被拉伸并量化为24个级别。 在2个最接近的表条目之间插入输出，然后仅更新最接近的条目。
条目被初始化为p = pr和n = 6（以减慢初始适配），且限制n <=255。APM与paq8l的不同之处在于，它使用新的StateMap快速初始适配，不更新两个相邻的表条目 ，并使用24个级别而不是33个级别。
这两个阶段使用离散的0阶上下文（最后0..7位）和哈希的1阶上下文（14位）。 每个输出取其输入加权1/4的平均值。
...
输出经过算术编码。 概率为p（s）的字符串s的代码是Q和Q + p（x）之间的数字，其中Q是按字典顺序在s之前所有字符串的总概率。 
该数字编码为big-endian base-256分数。 标头的前缀如下：
...
内存的三分之二（2 * 2 ^ N MB）用于将6个常规上下文（第1-4、6个字的顺序）映射到位历史的哈希表。 
查找每4位发生一次。 输入是面向字节的上下文，可能还有下一个字节的第一个半字节。 
输出是一个由15位历史记录组成的数组（每个1字节），用于通过附加0..3位形成的所有可能的上下文。 表条目的格式为：
...
第二个字节是结束于半字节边界的上下文的位历史。 它还是更换的优先事项。
状态通过增加总数进行排序，其中状态0表示初始状态（无历史记录）。
查找上下文时，会将8位校验和（哈希的一部分）与3个相邻条目进行比较，如果不匹配，则将清除优先级最低的条目并存储新的校验和。
...
哈希表在64字节高速缓存行上对齐。 查表永远不会越过64字节的地址边界。 
给定上下文的32位哈希h，将8位用于校验和，将17 + N位用于索引i。 然后尝试输入i，i XOR 1和i XOR 2。 
哈希h实际上是一个无冲突的置换，包括将输入乘以大的奇数mod 2 ^ 32、16位旋转和另一个乘法。

The order-1 context is mapped to a bit history using a 64K direct
lookup table, not a hash table.

使用64K直接查找表而不是哈希表将1阶上下文映射到位历史。
...
MatchModel使用三分之一的内存，在2 ^（N + 19）个字节的旋转输入缓冲区和带有2 ^（N + 17）个条目的索引（哈希表）之间平均分配。
保留了两个上下文哈希，一个长的h1长度为ceil（（N + 17）/ 3）个字节，一个短的h2长度为ceil（（N + 17）/ 5）个字节，其中ceil（） 是天花板功能。 该索引不使用冲突检测。
在每个字节边界处，如果当前没有匹配项，则将当前字节之前的字节与h1索引的位置进行比较。 如果少于2个字节匹配，则尝试使用h2。
如果找到长度等于或大于1的匹配项，则该匹配项将一直保持到下一个位与预测位不匹配为止。
表在每个字节之后的h1和h2处进行更新。
...
public:
  ...
  U32 predict(U32 state){
    return map[state];
  }
};

const U32 StaticStateMap::map[256] = {
...
static U32 hashHit = 0;
static U32 hashSearch = 0;
static U32 memHit = 0;
template <int B>
class HashTable {
...
HashTable<B>::HashTable(int n): t(0), N(n) {
  ...
  alloc(t, N+B*4+64);       // 多分配64B的空间用于后续的缓存行对齐
  t+=64-int(((long)t)&63);  // align on cache line boundary
}

inline U32 hashFun(U32 i){
  i*=123456791;
  i=i<<16|i>>16;
  i*=234567891;
  return i;
...
private:
  ...
  int c0=1;  // last 0-7 bits with leading 1
  int c4=0;  // last 4 bytes
  U8 *cp[6]={t0, t0, t0, t0, t0, t0};  // pointer to bit history
...
void Predictor::update(int y) {
  ...
  *cp[5]=nex(*cp[5], y);
  ...
  // staticMap
  // m.add(stretch(StaticStateMap::map[*cp[0]]));
  // m.add(stretch(StaticStateMap::map[*cp[1]]));
  // m.add(stretch(StaticStateMap::map[*cp[2]]));
  // m.add(stretch(StaticStateMap::map[*cp[3]]));
  // m.add(stretch(StaticStateMap::map[*cp[4]]));
  // m.add(stretch(StaticStateMap::map[*cp[5]]));
...
Encoder::Encoder(MODE m, char *out):
    mode(m), x1(0), x2(0xffffffff), x(0),pos(0),outBuf(out) {
